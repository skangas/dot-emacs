(custom-set-variables
 ...
 '(notmuch-saved-searches
   '((:name "inbox" :query "tag:inbox" :key
     ...
     (:name "emacs" :query "((tag:emacs-devel OR tag:emacs-bugs OR tag:emacs-diffs OR tag:emacs-help OR tag:emacs-todo OR tag:emacs-elpa-diffs) AND tag:unread) AND not tag:muted" :key
            [101])
     (:name "emacs-all" :query "(((tag:emacs-devel OR tag:emacs-bugs OR tag:emacs-diffs OR tag:emacs-help OR tag:emacs-elpa-diffs) AND tag:unread) OR tag:emacs-todo) AND not tag:muted" :key
            [102])
     (:name "debian" :query "tag:debian-mentors and tag:unread" :key
            [104])))
